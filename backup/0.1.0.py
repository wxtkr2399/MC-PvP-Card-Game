...
def set_language(language_: str) -> None:
    ...
    try:
        ...
        print(lang("message", "Failed to load language data"))
        lang_data = {}
    ...
    def AI_action(self) -> None:
        ...
        if self.AI_level == 1:  # 随机策略
            ...
            if card.need_target() and other_players:
                ...
                self(target)
                print(lang("message", "{player} attacks {target} with {card}", player=self.name, card=card.name, target=target.name))
            else:
                print(lang("message", "{player} used {card}", player=self.name, card=card.name))
            return
        ...
        if player.AI_level:
            ...
            if player.bedded:
                if player.bed_defence:
                    print(lang("message", "Bed state: be protected with {}", player.bed_defence[0]))
                else:
                    print(lang("message", "Bed state: Bedded"))
            else:
                print(lang("message", "Bed state: Unbedded"))
            print("...")
...
def main():
    ...
    game = Game()
    game.add_player(player, *[Player("", 3) for _ in range(3)])
