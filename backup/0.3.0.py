...
class Card:
    ...
    def usage(self) -> tuple[int, str]:
        ...
        if self.name == "Trident":
            return (3, DAMAGE_PHYSICAL)
        if self.name == "Damaged Trident":
...
def main():
    ...
    game = Game()
    game.add_player(Player("p1"), *(Player(f"p{i}") for i in range(2,5)))
...
def AI_competition():
    ...
    for ai in AI:
        players.extend((Player(ai + str(i)) for i in range(1, 5)))
