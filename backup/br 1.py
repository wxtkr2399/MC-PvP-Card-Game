...
class Card:
    ...
    def usage(self) -> tuple[int, str]:
        ...
        if self.name.endswith("Sword"):
            quality = self.name[:-5]
            damage = {"Wooden": 1, "Iron": 2, "Diamond": 3, "Netherite": 4}[quality.strip()]
    ...
    def need_target(self) -> bool:
        ...
        """
        return self.name.endswith("Sword") or self.name.endswith("Axe") or self.name == "TNT"
...
class Player:
    ...
    """
    def __init__(self, name: str, AI_level: int = 0) -> None:
        """初始化玩家
...
class CardPool:
    ...
    def __init__(self, cards: dict[Card, int] = defult.copy()) -> None:
        """初始化卡牌池
    ...
    def __str__(self) -> str:
        ...
        Returns:
            卡牌名称及数量的字符串
        """
        return ", ".join(f"{card.name} x{count}" for card, count in self.cards.items())
...
if __name__ == "__main__":
    ...
    for player in player_list:
        player.add_card(*card_pool.draw_card(5))
    ...
    while len(player_list) > 1:
        ...
        for player in player_list:
            ...
            print(f"Cards: {player.list_cards()}")
            print(f"Health: {player.health}")
            print(f"Effects: {player.effects}")
            print()

            if not player.cards:
                print(f"No cards left in {player.name}'s hand")
                player.add_card(*card_pool.draw_card(5))
                continue
            card = input("Enter card name to use: ")
            card = next((c for c in player.cards if c.name == card), None)
            if card is None:
                print("Invalid card name")
                continue
            player(card)
            if card.need_target():
                target = input("Enter target player name: ")
                target_player = next((p for p in player_list if p.name == target), None)
                if target_player is None:
                    print("Invalid target player name")
                    continue
                print(f"{player.name} attacks {target_player.name} with {card.name}")
                player(target_player)
                player.using = None
            else:
                print(f"{player.name} used {card.name}")
                player.using = None
            print()
            print()
        at()
        if not card_pool.cards:
            print("Card pool is empty, resetting...")
            card_pool.reset()
