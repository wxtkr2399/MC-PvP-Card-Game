...
class Card:
    ...
    def __init__(self, name: str) -> None:
        ...
        if not all(c.isalnum() or c.isspace() for c in name) or not name:
    ...
    def __init__(self, name: str | None = None, AI_level: int = 0) -> None:
        ...
        if name == None or name == "":
            with open(name_path, "r") as file:
                name = random.choice(file.readlines()).strip()
    ...
    def __call__(self, card_or_target: Card | "Player" | None = None) -> None:
        ...
        if isinstance(card_or_target, Card):
            ...
            if card_or_target in self.cards:
                ...
                if self.using.name in ("Shield") or self.using.name.endswith("Apple"):
...
class Game:
    ...
    def _handle_player_turn(self, player: Player) -> None:
        ...
        if player.AI_level:
            ...
            if action["card"]:
                if action["target"]:
                    messages["use_card"] = (lang("message", "{player} attacks {target} with {card}", player = player.name, card = action["card"], target = action["target"]))
                else:
                    messages["use_card"] = (lang("message", "{player} used {card}", player = player.name, card = action["card"]))

            if messages["use_card"]:
                print(messages["use_card"])
...
if __name__ == "__main__":
    ...
    game = Game()
    game.add_player(player1, player2, player3, player4)
    game.start()